<x-layout>
    <div class="max-w-3xl mx-auto my-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-8">Edytuj post</h1>

        <form method="POST" action="{{ route('posts.update', $post->id) }}" class="bg-white rounded-lg shadow-md p-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    <label for="title" class="block text-sm font-medium text-gray-700 mb-2">Tytuł *</label>
                ...
                <div>
                    <label for="lead" class="block text-sm font-medium text-gray-700 mb-2">Zajawka (podsumowanie)</label>
                    <textarea id="lead" name="lead" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none">{{ old('lead', $post->lead) }}</textarea>
                    @error('lead')
                        <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div>
                    <label for="content" class="block text-sm font-medium text-gray-700 mb-2">Treść *</label>
                    <textarea id="content" name="content" rows="10" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none">{{ old('content', $post->content) }}</textarea>
                    @error('content')
                        <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
